window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#region \0dsh-css:/home/runner/work/deepseek-harness/deepseek-harness/packages/client/ui-conversation/src/client/chat/LongTextCard.module.css.mjs
		const css$lt = ".LtTxt1_root{border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);border-radius:12px;flex-direction:column;display:flex;overflow:hidden}.LtTxt1_head{min-width:0;align-items:center;gap:8px;padding:8px 12px;display:flex}.LtTxt1_icon{color:var(--dsw-alias-label-secondary);flex:none;display:inline-flex}.LtTxt1_title{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:auto;font-size:13px;font-weight:500;line-height:20px;overflow:hidden}.LtTxt1_meta{color:var(--dsw-alias-label-caption);flex:none;font-size:12px;line-height:18px}.LtTxt1_actions{margin-left:8px;align-items:center;gap:4px;display:inline-flex}.LtTxt1_toggle{width:28px;height:28px;color:var(--dsw-alias-label-secondary);cursor:pointer;background:0 0;border:none;border-radius:8px;flex:none;place-items:center;display:grid}.LtTxt1_toggle:hover{background:var(--dsw-alias-interactive-bg-hover)}.LtTxt1_body{border-top:1px solid var(--dsw-alias-border-l1);padding:12px}";
		const tagId$lt = "@deepseek-ai/dsh-client-ui-conversation/LongTextCard.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$lt) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@deepseek-ai/dsh-client-ui-conversation";
			tag.dataset.pluginCss = tagId$lt;
			tag.textContent = css$lt;
			document.head.appendChild(tag);
		}
		var LongTextCard_module_css_default = {
			"root": "LtTxt1_root",
			"head": "LtTxt1_head",
			"icon": "LtTxt1_icon",
			"title": "LtTxt1_title",
			"meta": "LtTxt1_meta",
			"actions": "LtTxt1_actions",
			"toggle": "LtTxt1_toggle",
			"body": "LtTxt1_body"
		};
		//#endregion
		//#region lib/types/client/chat/LongTextCard.js
		/** Character threshold above which a settled message collapses into a .txt card. */
		const LONG_TEXT_CHARS = 2000;
		/** Trigger a browser download of the given plain text as a timestamped .txt file. */
		function downloadTextFile(text) {
			const stamp = new Date();
			const pad = (n) => String(n).padStart(2, "0");
			const name = `message-${stamp.getFullYear()}${pad(stamp.getMonth() + 1)}${pad(stamp.getDate())}-${pad(stamp.getHours())}${pad(stamp.getMinutes())}${pad(stamp.getSeconds())}.txt`;
		...
		/** Collapsed long-text card: header (download + expand) and an optional expanded body. */
		const LongTextCard = (0, react.memo)(function LongTextCard({ text, title, body, t }) {
			const [expanded, setExpanded] = (0, react.useState)(false);
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
							...
							(0, react_jsx_runtime.jsxs)("span", {
								...
								children: [
									...
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										className: LongTextCard_module_css_default.toggle,
										"aria-expanded": expanded,
										"aria-label": expanded ? t("message.longText.collapse") : t("message.longText.expand"),
										onClick: () => {
											setExpanded((value) => !value);
										},
										children: expanded ? (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronUpOutline14, {}) : (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.IconChevronDownOutline14, {})
									})
								]
							})
						]
					...
					expanded && (0, react_jsx_runtime.jsx)("div", {
						className: LongTextCard_module_css_default.body,
						children: body
					})
				]
			});
		...
		*/
		...
		const zh = {
			...
			"message.longText.reply": "长文本回复",
			"message.longText.input": "长文本输入",
			"message.longText.chars": "{n} 字",
		...
		const en = {
			...
			"message.longText.reply": "Long reply",
			"message.longText.input": "Long input",
			"message.longText.chars": "{n} chars",
